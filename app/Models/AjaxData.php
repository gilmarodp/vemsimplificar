...
class AjaxData extends Model
{
    ...
    public function getToEditResolution (array $section)
	{
        ...
        {
            $sql = "SELECT `content_question`, `resolution_question` FROM `" . PREFIX_DB . "resolutions_questions` WHERE `author` = :author AND `exam_year` = :exam_year AND `discipline` = :discipline_id AND `number_question` = :number_question AND `school` = :name_school";
            $stmt = $this->pdo->prepare($sql);
